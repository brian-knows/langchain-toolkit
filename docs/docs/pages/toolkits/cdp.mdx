...
The `CDPToolkit` is a `langchain` toolkit that can be imported in your agent to interact with the Brian APIs and execute operations on a Coinbase MPC-wallet.

## Usage

You can import directly the `CDPToolkit` in your agent without the need of using the `createBrianCDPAgent` function:
...
Every tool inside the `CDPToolkit` is an object of the following class that extends the `DynamicStructuredTool` class from `langchain`:
