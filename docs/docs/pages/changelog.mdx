...
- Added support for ZK chains to the `BrianToolkit` (only ZKSync for now)
- Fixed a typo in the [`Getting Started`](/getting-started) guide (thanks [@jarrodwatts](https://github.com/jarrodwatts))

## v1.2.8

- Added the possibility to pass custom tools to the agents via the `tools` option
- Add verification of deployed Smart Contracts by passing an `ETHERSCAN_API_KEY`

## v1.2.7

- Added the `TwitterToolkit` for publishing posts on Twitter, or fetching posts/latest trends on the social media
